...
export function Sidebar({ mobile = false, onNavigate }: { mobile?: boolean; onNavigate?: () => void }) {
  ...
  const removeCollection = async (id: string, name: string) => {
    if (!window.confirm(`删除“${name}”？其中资料会保留到未归档资料中。`)) return;
    try {
      await api.deleteCollection(id);
      if (scope.activeId === id) scope.selectCollection("");
      await scope.refresh();
    } catch (reason) {
      window.alert(reason instanceof Error ? reason.message : "删除文件夹失败");
    }
  };
