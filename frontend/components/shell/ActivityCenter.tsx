  ...
  return <div className="relative">
    ...
    {open && <>
      ...
      <div role="dialog" aria-label="任务中心" className="popover absolute right-0 top-12 z-50 w-80 max-w-[calc(100vw-24px)] rounded-appleLg border p-2">
        ...
        {items.length ? items.map((item) => <Link key={item.id} href={item.href} onClick={() => setOpen(false)} className="flex min-h-11 items-center gap-3 rounded-apple px-3 py-2 hover:bg-ink/[.05]"><span className={`h-2.5 w-2.5 rounded-full ${item.state === "running" ? "bg-status-warn" : item.state === "success" ? "bg-status-ok" : "bg-status-err"}`} /><span className="min-w-0 flex-1 truncate text-sm">{item.title}</span></Link>) : <p className="px-3 py-6 text-center text-sm text-muted">暂时没有后台任务</p>}
      </div>
    </>}
  </div>;
}
