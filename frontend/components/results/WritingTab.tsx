...
export function WritingTab() {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-center gap-2"><div className={`mr-auto inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs ${verification?.verdict === "pass" ? "bg-status-ok/10 text-status-ok" : verification?.verdict === "fail" ? "bg-status-err/10 text-status-err" : "bg-ink/[.06] text-muted"}`}>{verification?.verdict === "pass" ? <Check size={14} /> : <ShieldCheck size={14} />}{checking ? "正在检查" : verification?.verdict === "pass" ? "来源检查通过" : verification?.verdict === "fail" ? "有内容待修正" : "草稿 · 尚未校验"}</div><button onClick={() => void verify()} disabled={checking} className="btn-secondary"><ShieldCheck size={14} />检查来源</button><button onClick={() => void verify(true)} disabled={checking} className="btn-secondary"><WandSparkles size={14} />检查并修复</button><button onClick={() => void writeBack()} disabled={verification?.verdict !== "pass" || !hasArtifact || saving} className="btn-primary"><Save size={14} />{saving ? "保存中…" : "保存结论"}</button></div>{error && <div className="status-error"><AlertTriangle size={16} />{error}</div>}{saved && <div className="rounded-apple bg-status-ok/10 px-4 py-3 text-sm text-status-ok">结论已回写知识库。</div>}{!hasArtifact && verification?.verdict === "pass" && <div className="rounded-apple bg-status-warn/10 px-4 py-3 text-sm text-status-warn">回写前至少需要一个分析产物 ⟦art_*⟧ 锚点。</div>}<div className="grid min-h-[620px] overflow-hidden rounded-appleLg border bg-surface lg:grid-cols-2"><section className="flex min-h-[420px] flex-col border-b lg:border-b-0 lg:border-r"><div className="flex h-11 items-center border-b px-4 text-xs font-semibold text-muted"><span>正文编辑</span><span className="ml-auto">{text.length} 字符 · 自动保存</span></div><textarea value={text} onChange={(event) => { setText(event.target.value); setVerification(undefined); }} className="min-h-0 flex-1 resize-none bg-transparent p-6 font-mono text-sm leading-7 outline-none" spellCheck={false} /></section><section className="overflow-y-auto bg-canvas p-6"><div className="min-h-full rounded-apple border bg-surface p-6"><div className="mb-5 flex items-center gap-2 border-b pb-3 font-semibold"><ShieldCheck size={16} className="text-brand" />可信预览</div><AnchoredMarkdown text={text} />{verification?.items.some((item) => item.verdict === "fail") && <div className="mt-7 border-t pt-4"><h3 className="text-sm font-semibold text-status-err">需要处理</h3><div className="mt-3 space-y-2">{verification.items.filter((item) => item.verdict === "fail").map((item, index) => <div key={index} className="rounded-apple bg-status-err/10 p-3 text-xs text-status-err"><strong>{item.check}</strong> · {item.reason}</div>)}</div></div>}</div></section></div></div>;
}
