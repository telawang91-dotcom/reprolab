...
export function ArtifactsTab() {
  ...
  return <div className="space-y-6"><section className="grid gap-4 sm:grid-cols-3">{[{ label: "成功运行", value: review.counts.successful_runs }, { label: "分析产物", value: review.counts.artifacts }, { label: "可信结论", value: review.counts.verified_claims }].map((item) => <article key={item.label} className="rounded-appleLg border bg-surface p-5 shadow-soft"><strong className="text-3xl font-semibold tracking-tight">{item.value}</strong><span className="mt-1 block text-sm text-muted">{item.label}</span></article>)}</section><section><div className="mb-4 flex items-end justify-between"><div><h2 className="text-xl font-semibold">成果库</h2><p className="mt-1 text-sm text-muted">每个结果都可检查来源、导出或进入写作。</p></div><span className="text-xs text-status-ok">{items.filter((item) => item.source_complete).length} 项来源完整</span></div>{items.length ? <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">{items.map((item) => <article key={item.id} className="group rounded-appleLg border bg-surface p-4 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift"><div className="flex items-start gap-3"><span className={`grid h-9 w-9 place-items-center rounded-appleSm ${item.source_complete ? "bg-status-ok/10 text-status-ok" : "bg-status-warn/10 text-status-warn"}`}>{item.source_complete ? <CheckCircle2 size={17} /> : <AlertTriangle size={17} />}</span><div className="min-w-0"><span className="text-xs uppercase tracking-wide text-subtle">{item.kind}</span><h3 className="truncate font-semibold">{item.title || "未命名分析产物"}</h3></div></div>{item.kind === "figure" && item.content_hash ? <img src={api.artifactContentUrl(item.id)} alt={item.title || "分析图表"} className="mt-4 h-44 w-full rounded-apple object-contain" /> : <pre className="mt-4 h-44 overflow-auto rounded-apple bg-ink/[.04] p-3 font-mono text-xs">{JSON.stringify(item.value, null, 2)}</pre>}<div className="mt-4 flex gap-2"><Link href={`/lineage/${item.id}`} className="btn-secondary h-9 flex-1 px-3"><Network size={13} />来源</Link><button onClick={() => exportArtifact(item)} className="btn-secondary h-9 px-3" aria-label="导出产物"><Download size={13} /></button><Link href="/results?tab=writing" onClick={() => addArtifactToWriting(activeProjectId(), { artifact_id: item.id, title: item.title })} className="btn-primary h-9 px-3"><PenLine size={13} />写作</Link></div></article>)}</div> : <div className="rounded-appleLg border border-dashed p-12 text-center"><BarChart3 className="mx-auto text-subtle" /><h3 className="mt-4 font-semibold">还没有分析产物</h3><p className="mt-2 text-sm text-muted">完成一次真实数据分析后，图、表和数字会出现在这里。</p><Link href="/analysis" className="btn-primary mt-5">开始分析</Link></div>}</section></div>;
}
