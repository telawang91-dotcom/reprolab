...
export function VerifiedPreview({ text, items, onAnchor }: { text: string; items: VerifyItem[]; onAnchor: (anchor: string) => void }) {
  ...
  return <div><div className="whitespace-pre-wrap leading-7">{parts.map((part, index) => part.failure ? <span key={index} title={part.failure.reasons.join("；")} className="decoration-wavy decoration-red-500 decoration-2 underline underline-offset-4"><AnchoredMarkdown text={part.text} onAnchor={onAnchor}/></span> : <AnchoredMarkdown key={index} text={part.text} onAnchor={onAnchor}/>)}</div>{evidence.length > 0 && <div className="mt-5 space-y-2 border-t pt-4">{evidence.map((item, index) => <details key={`${item.target_anchor}-${index}`} className="rounded-lg border bg-slate-50 p-3 text-xs"><summary className="cursor-pointer font-medium"><button type="button" onClick={(event) => { event.preventDefault(); if (item.target_anchor) onAnchor(item.target_anchor); }} className="mr-2 font-mono text-brand">{item.target_anchor}</button>{item.label} · {(item.support_score ?? 0).toFixed(2)} · 展开证据段落</summary><p className="mt-3 whitespace-pre-wrap leading-5 text-slate-600">{item.evidence_span}</p></details>)}</div>}</div>;
}
