...
export function ArtifactPanel({ artifact, total, onLineage, onSave }: { artifact?: TimelineArtifact; total: number; onLineage: (id: string) => void; onSave: SaveSkill }) {
  ...
  function openHarvest() {
    if (!artifact) return;
    setName(artifact.title?.trim() || `${artifact.kind} 分析技能`);
  ...
  async function save() {
    ...
    try { await onSave(artifact, name.trim(), intent.trim(), discipline); setHarvestOpen(false); }
    catch (reason) { setSaveError(reason instanceof Error ? reason.message : "技能沉淀失败"); }
  ...
  return <>
    <motion.div layoutId={`artifact-${artifact.artifact_id}`} transition={reduceMotion ? { duration: 0 } : agentSpring} className="space-y-4">
      <div className="flex items-center gap-2"><BarChart3 size={16} className="text-brand"/><strong className="min-w-0 flex-1 truncate text-sm">{artifact.title || `${artifact.kind} 产物`}</strong><span className="text-xs text-muted">{total} 项</span></div>
      <div className="overflow-hidden rounded-apple border bg-surface">{artifact.figure_url ? <img src={api.artifactContentUrl(artifact.artifact_id)} alt={artifact.title || "当前分析产物"} className="max-h-72 w-full object-contain p-3"/> : <pre className="max-h-72 overflow-auto bg-ink/[.035] p-4 font-mono text-xs">{JSON.stringify(unwrapArtifactValue(artifact.value_json), null, 2)}</pre>}<div className="border-t px-3 py-2 font-mono text-xs text-brand">{artifact.anchor}</div></div>
    ...
    <Sheet open={harvestOpen} onOpenChange={setHarvestOpen} title="沉淀为可复用技能" side="bottom">
      <div className="mx-auto max-w-2xl space-y-5">
        <div className="flex items-start gap-3 rounded-apple bg-status-ok/[.08] p-4 text-sm text-status-ok"><ShieldCheck size={17} className="mt-0.5 shrink-0"/><span><strong>可信来源已锁定。</strong> 系统会从运行代码识别语义字段，保留 DATASET_PATHS 与产物登记协议；应用到新数据时仍会重新执行。</span></div>
        ...
        <div className="flex justify-end gap-2"><button onClick={() => setHarvestOpen(false)} className="btn-secondary">取消</button><button disabled={!name.trim() || !intent.trim() || saving} onClick={() => void save()} className="btn-primary">{saving ? <Loader2 size={14} className="animate-spin"/> : <BookmarkPlus size={14}/>} {saving ? "正在识别字段…" : "确认沉淀"}</button></div>
      </div>
    </Sheet>
  </>;
}
