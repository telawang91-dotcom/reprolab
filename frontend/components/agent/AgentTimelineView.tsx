...
export function AgentTimelineView({ timeline, hasDatasets, hasSelection, onExample, onAnchor, onArtifact }: { timeline: AgentTimeline; hasDatasets: boolean; hasSelection: boolean; onExample: (text: string) => void; onAnchor: (anchor: string) => void; onArtifact: (artifact: TimelineArtifact) => void }) {
  ...
  return <div className="space-y-4"><PlanTracker steps={timeline.steps} /><AnimatePresence initial={false}>{timeline.steps.map((step, index) => <StepBlock key={step.id} step={step} index={index} onArtifact={onArtifact} />)}</AnimatePresence>{conclusions.map((conclusion, index) => <motion.div key={`${index}-${conclusion.text.slice(0, 24)}`} initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={agentSpring}><div className="mb-2 text-xs font-semibold text-muted">第 {index + 1} 轮结论</div><ConclusionCard text={conclusion.text} onAnchor={onAnchor} /></motion.div>)}</div>;
}
