...
function UserQuestion({ text }: { text: string }) {
  return (
    <div className="ml-auto max-w-[82%] rounded-appleLg rounded-br-md bg-ink px-4 py-3 text-sm leading-6 text-white">
      {text}
    </div>
  );
}
...
export function AgentTimelineView({
      ...
      <div className="grid min-h-[52vh] place-items-center text-center">
        <div className="max-w-xl">
          ...
          <h2 className="mt-5 text-2xl font-semibold tracking-tight">
            {hasDatasets ? "从一个科研问题开始" : "先添加一份可分析的数据"}
          </h2>
          <p className="mt-3 text-sm leading-6 text-muted">
            {hasDatasets
              ? hasSelection
                ? "描述你要比较、检验或解释什么。运行时展示分析过程，完成后聚焦最终回答。"
                : "先选择数据集，避免生成没有来源的分析。"
              : "上传 CSV 或 XLSX 后，Agent 会按问题动态生成代码并登记完整血缘。"}
          </p>
          {hasDatasets ? (
            <div className="mt-6 grid gap-2 sm:grid-cols-3">
              {["比较不同组的均值并绘图", "分析两个变量的相关性", "建立回归模型并解释系数"].map((prompt) => (
                <button key={prompt} disabled={!hasSelection} onClick={() => onExample(prompt)} className="rounded-apple border bg-surface p-3 text-left text-sm hover:border-brand/35 disabled:opacity-40">
                  {prompt}
                </button>
              ))}
            </div>
  ...
  const conclusions = timeline.conclusions.length
    ...
    <div className="space-y-4">
      ...
      {running && (
        <section className="space-y-4" aria-label="当前分析过程">
          <PlanTracker steps={liveTimeline.steps} />
          <AnimatePresence initial={false}>
            {liveTimeline.steps.map((step, index) => (
              <StepBlock key={step.id} step={step} index={index} onArtifact={onArtifact} />
            ))}
          </AnimatePresence>
        </section>
      )}
