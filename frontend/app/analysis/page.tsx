...
import { AlertTriangle, BarChart3, BookmarkPlus, Check, ChevronDown, ChevronRight, Code2, Database, ExternalLink, FileSpreadsheet, Play, RotateCcw, Send, Sparkles, Terminal, X } from "lucide-react";
  ...
  return <div className="flex h-[calc(100vh-48px)] min-h-[640px] overflow-hidden">
    ...
    <main className="relative flex min-w-0 flex-1 flex-col bg-[#F8FAFC] dark:bg-[#0B0F17]">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 md:px-8"><div className="mx-auto max-w-3xl">
        ...
        {events.length === 0 ? <div className="grid min-h-[55vh] place-items-center text-center"><div><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-blue-50 text-brand dark:bg-blue-950"><Sparkles size={24}/></span><h2 className="mt-4 text-xl font-semibold">从一个科研问题开始</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">选择数据后，用自己的话说明要比较、检验或解释什么。系统会运行分析，并保存每个数字和图的来源。</p><div className="mt-5 flex flex-wrap justify-center gap-2">{examples.map((item) => <button key={item} onClick={() => setMessage(item)} className="rounded-full border bg-white px-3 py-2 text-xs text-slate-600 hover:border-blue-300 hover:text-brand dark:bg-slate-900">{item}</button>)}</div></div></div> : <div className="space-y-4">{events.map((item) => <EventCard key={item.id} item={item} onArtifact={(artifact) => setActiveArtifact(artifact)} onAnchor={anchorClick} onLineage={showLineage}/>)}</div>}
      ...
      <div className="shrink-0 border-t bg-white p-4 dark:border-slate-800 dark:bg-slate-950"><div className="mx-auto max-w-3xl"><div className="rounded-xl border bg-white p-2 shadow-card focus-within:border-blue-300 dark:border-slate-700 dark:bg-slate-900"><textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if ((e.metaKey || e.ctrlKey) && e.key === "Enter") void send(); }} rows={2} className="w-full resize-none bg-transparent px-2 py-1 outline-none" placeholder={selected.length ? "描述你想分析的问题…" : "可先选择数据集，再描述分析问题…"}/><div className="flex items-center px-1 pt-1"><span className="text-[11px] text-slate-400">⌘↵ 发送 · 所有产物自动登记溯源</span>{running ? <button onClick={() => abortRef.current?.abort()} className="btn-secondary ml-auto h-8"><X size={14}/>停止</button> : <button onClick={() => void send()} disabled={!message.trim()} className="btn-primary ml-auto h-8"><Send size={14}/>发送</button>}</div></div></div></div>
    </main>
...
function ArtifactPreview({ artifact, onLineage, onSave }: { artifact: ArtifactData; onLineage: (id: string) => void; onSave: (artifact: ArtifactData) => void }) {
  return <motion.div key={artifact.artifact_id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5"><div className="card overflow-hidden"><div className="flex items-center border-b px-4 py-3"><BarChart3 size={16} className="mr-2 text-brand"/><span className="font-medium">{artifact.kind}</span><span className="ml-auto font-mono text-xs text-brand">{artifact.anchor}</span></div>{artifact.figure_url ? <img className="max-h-72 w-full object-contain p-3" src={`${API_BASE.replace(/\/api\/v1$/, "")}${artifact.figure_url}`} alt="当前产物"/> : <pre className="max-h-72 overflow-auto p-4 font-mono text-xs">{JSON.stringify(artifact.value_json, null, 2)}</pre>}</div><button onClick={() => void onSave(artifact)} className="btn-secondary mt-3 w-full"><BookmarkPlus size={14}/>存为我的技能</button><div className="mt-2 grid grid-cols-2 gap-2"><Link href={`/lineage/${artifact.artifact_id}`} className="btn-primary"><ExternalLink size={14}/>全屏溯源</Link><button onClick={() => void onLineage(artifact.artifact_id)} className="btn-secondary"><RotateCcw size={14}/>快速查看</button></div><div className="mt-4 rounded-lg border bg-slate-50 p-3 dark:bg-slate-900"><div className="label">Artifact ID</div><div className="mt-1 break-all font-mono text-[11px] text-slate-500">{artifact.artifact_id}</div></div></motion.div>;
}
