...
import { api, streamChat, type ChatEvent, type DocumentDetail, type Lineage, type SkillItem } from "@/lib/api";
...
const examples = [
  { label: "组间差异", prompt: "比较不同组的均值并绘图", detail: "箱线图 + 显著性检验" },
  { label: "变量关系", prompt: "分析两个变量的相关性", detail: "散点图 + 相关性" },
  { label: "解释模型", prompt: "建立回归模型并解释系数", detail: "回归 + 系数解读" },
];
...
export default function AnalysisPage() {
  ...
  return <div className="flex h-[calc(100vh-48px)] min-h-[640px] overflow-hidden">
    ...
    <main className="relative flex min-w-0 flex-1 flex-col bg-[#F8FAFC] dark:bg-[#0B0F17]">
      <div className="flex h-14 shrink-0 items-center border-b bg-white px-4 dark:border-slate-800 dark:bg-slate-950 md:px-5"><div><div className="font-semibold">分析对话</div><div className="hidden text-xs text-slate-400 sm:block">先确认数据范围，再查看计划、代码和结果来源</div></div><button onClick={() => setMobileDataOpen(true)} className="btn-secondary ml-auto h-8 lg:hidden"><Database size={14}/>数据集 {selected.length ? `· ${selected.length}` : ""}</button><div className="ml-auto hidden items-center gap-2 text-xs text-slate-400 lg:flex"><ShieldCheck size={14} className="text-emerald-600"/>运行将登记数据、代码与环境</div></div>
      ...
      <div className="shrink-0 border-t bg-white p-4 dark:border-slate-800 dark:bg-slate-950"><div className="mx-auto max-w-3xl"><div className="rounded-xl border bg-white p-2 shadow-card focus-within:border-blue-300 dark:border-slate-700 dark:bg-slate-900"><textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if ((e.metaKey || e.ctrlKey) && e.key === "Enter" && selected.length) requestAnalysis(); }} rows={2} className="w-full resize-none bg-transparent px-2 py-1 outline-none" placeholder={selected.length ? "描述你想分析的问题…" : "先选择一个数据集…"}/><div className="flex items-center px-1 pt-1"><span className="text-[11px] text-slate-400">{selected.length ? "⌘↵ 确认范围 · 运行前可检查数据和问题" : "需要先选择数据集，避免生成无来源分析"}</span>{running ? <button onClick={() => abortRef.current?.abort()} className="btn-secondary ml-auto h-8"><X size={14}/>停止</button> : <button onClick={requestAnalysis} disabled={!message.trim() || !selected.length} className="btn-primary ml-auto h-8"><Send size={14}/>确认并开始</button>}</div></div></div></div>
    </main>
    ...
    {confirmationOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-4" onMouseDown={() => setConfirmationOpen(false)}><section role="dialog" aria-modal="true" aria-label="确认分析范围" className="card w-full max-w-xl p-5" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-brand"><ShieldCheck size={18}/></span><div><div className="label">运行前确认</div><h2 className="mt-1 text-lg font-semibold">确认本次分析范围</h2><p className="mt-1 text-sm leading-6 text-slate-500">确认后，Agent 才会生成执行计划和代码；完整过程会保存到可信记录。</p></div><button onClick={() => setConfirmationOpen(false)} aria-label="关闭确认" className="btn-secondary ml-auto h-8 w-8 px-0"><X size={14}/></button></div><dl className="mt-5 space-y-3"><div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-900"><dt className="label">研究问题</dt><dd className="mt-1 text-sm leading-6">{message}</dd></div><div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-900"><dt className="label">输入数据</dt><dd className="mt-2 space-y-1 text-sm">{selectedDatasets.map((item) => <div key={item.id}>{item.filename} <span className="text-xs text-slate-400">· {item.schema_json?.row_count ?? 0} 行 / {item.schema_json?.column_count ?? 0} 列</span></div>)}</dd></div><div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800"><strong>将被记录：</strong>输入数据版本、生成代码、固定随机种子和环境快照。</div></dl><div className="mt-5 flex justify-end gap-2"><button onClick={() => setConfirmationOpen(false)} className="btn-secondary">返回修改</button><button onClick={() => { setConfirmationOpen(false); void send(); }} className="btn-primary"><Send size={14}/>确认并运行</button></div></section></div>}
...
function ArtifactPreview({ artifact, onLineage, onSave }: { artifact: ArtifactData; onLineage: (id: string) => void; onSave: (artifact: ArtifactData) => void }) {
  const [trustState, setTrustState] = useState<"checking" | "complete" | "incomplete">("checking");
  useEffect(() => {
    let live = true;
    setTrustState("checking");
    api.lineage(artifact.artifact_id).then((result) => {
      const types = new Set(result.nodes.map((node) => node.type));
      if (live) setTrustState(types.has("dataset") && types.has("run") && types.has("artifact") ? "complete" : "incomplete");
    }).catch(() => { if (live) setTrustState("incomplete"); });
    return () => { live = false; };
  ...
  return <motion.div key={artifact.artifact_id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5"><div className="overflow-hidden border-y"><div className="flex items-center py-3"><BarChart3 size={16} className="mr-2 text-brand"/><span className="font-medium">{artifact.kind}</span><span className="ml-auto font-mono text-xs text-brand">{artifact.anchor}</span></div>{artifact.figure_url ? <img className="max-h-72 w-full object-contain py-3" src={api.artifactContentUrl(artifact.artifact_id)} alt="当前产物"/> : <pre className="max-h-72 overflow-auto bg-slate-50 p-4 font-mono text-xs dark:bg-slate-900">{JSON.stringify(artifact.value_json, null, 2)}</pre>}</div><div className={`mt-4 border-l-2 px-3 py-3 ${complete ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30" : "border-amber-500 bg-amber-50 dark:bg-amber-950/30"}`}><div className={`flex items-center gap-2 text-sm font-medium ${complete ? "text-emerald-800 dark:text-emerald-300" : "text-amber-900 dark:text-amber-200"}`}><ShieldCheck size={15}/>{trustState === "checking" ? "正在核对可信记录" : complete ? "可信记录完整" : "可信记录待补全"}</div><div className={`mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] ${complete ? "text-emerald-700 dark:text-emerald-400" : "text-amber-800 dark:text-amber-300"}`}>{complete ? <><span>✓ 输入数据</span><span>✓ 运行代码</span><span>✓ 环境快照</span></> : <span>请查看完整来源，确认数据、运行与产物链路后再写入结论。</span>}</div></div><Link href="/writing" onClick={prepareWriting} className="btn-primary mt-4 w-full"><PenLine size={14}/>写入研究结论</Link><div className="mt-2 grid grid-cols-2 gap-2"><Link href={`/lineage/${artifact.artifact_id}`} className="btn-secondary"><ExternalLink size={14}/>查看完整来源</Link><button onClick={() => void onSave(artifact)} className="btn-secondary"><BookmarkPlus size={14}/>存为技能</button></div><button onClick={() => void onLineage(artifact.artifact_id)} className="mt-3 inline-flex items-center gap-1 text-xs text-slate-400 hover:text-brand"><RotateCcw size={12}/>快速检查血缘</button><details className="mt-4 border-t pt-3 text-xs text-slate-400"><summary className="cursor-pointer">技术信息</summary><div className="mt-2 break-all font-mono text-[10px]">artifact {artifact.artifact_id}</div></details></motion.div>;
}
