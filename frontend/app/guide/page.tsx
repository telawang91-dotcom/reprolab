import { ArrowRight, BookOpen, Brain, Database, FileCheck2, FlaskConical, HelpCircle, Lightbulb, Network, ShieldCheck } from "lucide-react";
...
export default function GuidePage() {
  return <div className="page-shell max-w-6xl space-y-12">
    <section className="rounded-appleXl bg-ink px-7 py-12 text-white sm:px-12 sm:py-16"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.12em] text-white/60"><HelpCircle size={15}/>Product guide</div><h1 className="mt-5 max-w-3xl text-4xl font-semibold tracking-[-.045em] sm:text-5xl">从资料到可信结论，<br/>每一步都有来源。</h1><p className="mt-5 max-w-2xl text-[17px] leading-7 text-white/70">创建研究项目、导入任意资料，再像聊天一样提出问题。ReproLab 会按需检索、运行分析，并把值得保留的结果交给你决定。</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/knowledge" className="btn-primary">导入研究资料<ArrowRight size={15}/></Link><Link href="/analysis" className="btn border border-white/20 bg-white/10 text-white hover:bg-white/15">进入研究 Agent<ArrowRight size={15}/></Link></div></section>

    <section><div className="eyebrow text-brand">使用路径</div><div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="section-title">按四步完成一次研究闭环</h2><p className="mt-2 text-[17px] text-muted">从真实资料开始，最终只沉淀你确认有价值的成果。</p></div><Link href="/knowledge" className="text-link text-[17px]">从资料入库开始<ArrowRight size={14}/></Link></div><div className="mt-6 grid gap-4 md:grid-cols-2">{workflow.map(({ number, title, text, href, icon: Icon }) => <Link key={number} href={href} className="interactive-card group flex gap-4"><span className="text-3xl font-semibold text-subtle group-hover:text-brand">{number}</span><span className="grid h-10 w-10 shrink-0 place-items-center rounded-appleSm bg-brand/10 text-brand"><Icon size={18}/></span><span><strong className="block font-semibold">{title}</strong><span className="mt-1 block text-sm leading-6 text-muted">{text}</span><span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-brand">前往此步骤<ArrowRight size={13}/></span></span></Link>)}</div></section>
