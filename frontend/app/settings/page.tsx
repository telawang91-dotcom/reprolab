...
export default function SettingsPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6 p-5 lg:p-8">
    ...
    {notice && <div role="status" className={`flex items-center gap-2 rounded-xl border px-4 py-3 text-sm ${notice.kind === "success" ? "border-status-ok/20 bg-status-ok/[.06] text-status-ok" : "border-status-error/20 bg-status-error/[.06] text-status-error"}`}>{notice.kind === "success" ? <Check size={16}/> : <CircleHelp size={16}/>}<span>{notice.text}</span><button onClick={() => setNotice(undefined)} className="interactive ml-auto text-xs underline">关闭</button></div>}
    ...
    <section className="grid gap-5 lg:grid-cols-[220px_1fr]">
      ...
      <form onSubmit={submit} className="card overflow-hidden">
        ...
        {loading || !config ? <div className="flex min-h-72 items-center justify-center gap-2 text-sm text-subtle"><Loader2 size={16} className="animate-spin"/>读取模型设置…</div> : <div className="space-y-5 p-5">
          ...
          {testResult && <div className={`rounded-xl border p-4 text-sm ${testResult.ok ? "border-status-ok/20 bg-status-ok/[.06] text-status-ok" : "border-status-error/20 bg-status-error/[.06] text-status-error"}`}><div className="font-medium">{testResult.ok ? "连接测试通过" : "连接测试失败"}</div><p className="mt-1">{testResult.message}</p><p className="mt-2 text-xs opacity-70">{testResult.model} · {testResult.latency_ms} ms</p></div>}
...
function StatusCard({ icon: Icon, title, state }: { icon: typeof Server; title: string; state: "checking" | "online" | "offline" }) {
  return <article className="interactive-card card p-4"><div className="flex items-center gap-2 text-sm font-medium"><Icon size={16} className={state === "online" ? "text-status-ok" : "text-subtle"}/>{title}<span className={`ml-auto h-2 w-2 rounded-full ${state === "online" ? "bg-status-ok" : state === "offline" ? "bg-status-error" : "animate-pulse bg-status-warn"}`}/></div><p className="mt-3 text-sm text-muted">{state === "online" ? "运行正常" : state === "offline" ? "暂时无法连接" : "正在检查…"}</p></article>;
}
...
function RuntimeCard({ item }: { item: RuntimeStatus["components"][number] }) {
  ...
  return <article className="interactive-card card p-4"><div className="flex items-center gap-2 text-sm font-medium"><span className={`h-2 w-2 rounded-full ${state === "online" ? "bg-status-ok" : state === "offline" ? "bg-status-error" : "bg-status-warn"}`}/>{item.title}</div><p className="mt-3 text-sm leading-6 text-muted">{item.message}</p>{item.action && <p className="mt-3 text-xs font-medium text-brand">下一步：{item.action}</p>}</article>;
}
