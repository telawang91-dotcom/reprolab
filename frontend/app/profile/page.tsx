...
export default function ProfilePage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 p-5 lg:p-8">
    <header><div className="label">账户与偏好</div><h1 className="mt-1 text-2xl font-semibold">个人资料</h1><p className="mt-2 text-sm text-muted">用于显示称呼和研究背景，帮助工作区提供更贴近你的默认体验。</p></header>

    <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
      <aside className="card h-fit p-6 text-center"><div className="mx-auto grid h-20 w-20 place-items-center rounded-2xl bg-ink text-xl font-semibold text-canvas">{profileInitials(profile.name)}</div><h2 className="mt-4 text-lg font-semibold">{profile.name || "研究者"}</h2><p className="mt-1 text-sm text-muted">{profile.role || "未填写身份"}</p>{profile.institution && <p className="mt-1 text-xs text-subtle">{profile.institution}</p>}<div className="mt-5 rounded-xl bg-ink/[.04] p-3 text-left text-xs leading-5 text-muted"><div className="mb-1 flex items-center gap-2 font-medium text-ink"><ShieldCheck size={14}/>本地工作区资料</div>当前版本不会公开展示这些信息，也不会用于训练模型。</div></aside>

      <form onSubmit={submit} className="card overflow-hidden"><div className="flex items-start gap-3 border-b p-5"><span className="grid h-10 w-10 place-items-center rounded-xl bg-brand/10 text-brand"><UserRound size={18}/></span><div><h2 className="font-semibold">基本信息</h2><p className="mt-1 text-sm text-muted">这些信息只保存在当前浏览器。</p></div></div><div className="space-y-5 p-5">
    ...
    <section className="rounded-xl border border-brand/20 bg-brand/[.06] p-5 text-sm leading-6"><div className="flex items-center gap-2 font-medium text-brand"><FlaskConical size={16}/>关于个性化</div><p className="mt-2">研究方向不会限制可用功能。ReproLab 的分析机制始终保持学科无关，这些资料只用于改善称呼、默认提示和建议排序。</p></section>
  </div>;
}
